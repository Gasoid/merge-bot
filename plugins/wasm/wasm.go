...
const (
	commandCtxKey commandContext = "command"
)
...
func BuildWasmPlugin(manifestFile []byte, vars map[string][]string) (plugins.HandlerFunc, error) {
	...
	compiledPlugin, err := extism.NewCompiledPlugin(ctx, extismManifest, config, []extism.HostFunction{getGitFile, searchCode, fetchWebContent, getCIJobLog})
	if err != nil {
		return nil, err
	}
